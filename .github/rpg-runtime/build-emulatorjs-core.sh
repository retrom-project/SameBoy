...
emmake make -C libretro -f Makefile clean platform=emscripten "$@"
emmake make -C libretro -j"4" -f Makefile platform=emscripten \
  INITIAL_HEAP=268435456 AUTO_MEMORY_GROWTH=1 "$@"

archive=$(find build/bin -maxdepth 1 -type f -name "${core_name}_libretro_emscripten.bc" -print)
